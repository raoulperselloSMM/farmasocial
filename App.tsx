...
const App: React.FC = () => {
  ...
  const handleSavePost = async (e: React.FormEvent) => {
    ...
    if (editingPostId) {
        ...
        const updatedPost: Post = {
            ...
            createdAt: posts.find(p => p.id === editingPostId)?.createdAt || Date.now(),
        };
        const updatedPosts = await storageService.updatePost(updatedPost);
        ...
        const updatedPosts = await storageService.savePost(newPost);
        setPosts(updatedPosts);
  ...
  const handleCreateCategory = async (e: React.FormEvent) => {
    ...
    const updatedCategories = await storageService.saveCategory(newCategory);
  ...
  const handleDeletePost = async (id: string) => {
    if (confirm("Sei sicuro di voler eliminare questo contenuto? L'azione è irreversibile.")) {
        const updatedPosts = await storageService.deletePost(id);
        setPosts(updatedPosts);
  ...
  const handleDeleteCategory = async (id: string) => {
    ...
    if (confirm(confirmMessage)) {
        const updatedCategories = await storageService.deleteCategory(id);
        setCategories(updatedCategories);
